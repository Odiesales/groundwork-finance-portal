...
snapshot_date = st.date_input('Reporting / Snapshot Date', value=date.today(), help='Use the Monday reporting date for the weekly portal refresh.')

section('Upload Center', 'Choose the Monday reporting date, then upload the AR and/or Revenue export. Existing snapshots with the same date will be replaced.')
...
with right:
    ...
    if rev_file:
        try:
            ...
            rev_target = REVENUE_SNAPSHOT_DIR / f'revenue_{snapshot_date:%Y-%m-%d}.csv'
            if rev_target.exists():
                st.warning('A Revenue snapshot already exists for this date. Saving will replace it.')
            if st.button('Save Revenue Snapshot', type='primary', key='save_revenue_snapshot'):
                ...
                rev_df.to_csv(rev_target, index=False)
            ...
            st.dataframe(rev_df.head(200), width='stretch', hide_index=True)
        except Exception as exc:
...
c1, c2 = st.columns(2)
c1.dataframe(a, width='stretch', hide_index=True)
c2.dataframe(r, width='stretch', hide_index=True)
