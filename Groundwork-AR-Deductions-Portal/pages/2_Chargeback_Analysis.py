...
with c1:
    ...
    fig = go.Figure(go.Bar(x=top.values, y=top.index, orientation="h", marker_color=YELLOW, text=[format_money(v, 0) for v in top.values], textposition="inside"))
...
with c2:
    ...
    fig = go.Figure(go.Bar(x=mix.values, y=mix.index, orientation="h", marker_color=YELLOW, text=[format_money(v, 0) for v in mix.values], textposition="inside"))
    fig.update_xaxes(tickformat="$,.0f")
    st.plotly_chart(chart_layout(fig, height=380), width="stretch")
...
fig = go.Figure(go.Scatter(x=trend["Month Date"], y=trend["Open Balance"], mode="lines+markers+text", text=[format_money(v, 0) for v in trend["Open Balance"]], textposition="top center", line=dict(color=YELLOW, width=3)))
